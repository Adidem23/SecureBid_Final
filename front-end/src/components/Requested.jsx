  ...
  useEffect(() =>{
    ...
    const getRequested = async () =>{
      ...
      for(let i=0; i<_reqIndices; i++){
        ...
        const reqLand = await contract.getRequestedTenders(i, {from: account});

        // if surveyNo. != 0
        if(reqLand[3].words[0] != 0){
          const landDetails = await contract.getTenderDetails(reqLand[0], reqLand[1], reqLand[2], reqLand[3].words[0], {
            from: account
          })
