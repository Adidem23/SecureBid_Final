    ...
    const getRequests = async () => {
      ...
      for (let i = 0; i < _totalIndices; i++) {
        ...
        if (ownerOwns[3].words[0] != 0) {
          ...
          const tenderId = reqCnt_propId[1].words[0];
          ...
          if (noOfRequests > 0) {
            ...
            for (let j = 0; j < noOfRequests; j++) {
              ...
              const landDetails = await contract.getTenderDetails('M', 'N', 'Y',ownerOwns[3], {
                from: account
              });
              ...
              const reqDetails = { state: ownerOwns[0], district: ownerOwns[1], city: ownerOwns[2], surveyNo: ownerOwns[3].words[0], index: i, reqNo: j, requesterName: requesterName, stringBidAmount: stringBidAmount, requesterFileURI: requesterFileURI, establishmentyear: establishmentyear, requester, tenderId , tendorName:tendorName}
